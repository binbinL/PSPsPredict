...
from MyNet import MyModel_woMH # type: ignore
from LrScheduler import WarmupLR # type: ignore
from Func import analysis # type: ignore
from config import train_root,val_root, batch_size,n_epoch,lr,dropout,save_root,d_model,n_class,vocab_size,dim_feedforward,d_embedding,kmer # type: ignore
...
model = MyModel_woMH(d_embedding, d_model, dropout, n_class, vocab_size,dim_feedforward,kmer)
model = model.to(device)
criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(),lr=lr, weight_decay=0.2)
lr_scheduler = WarmupLR(optimizer=optimizer, num_warm=30)
